...
class VoiceProxyHandler:
    ...
    async def _send_initial_config(
        ...
        # Send initial greeting to make the assistant speak first
        await self._send_initial_greeting(azure_ws)
    ...
    async def _send_initial_greeting(self, azure_ws: websockets.asyncio.client.ClientConnection) -> None:
        "Send an initial user message to trigger the assistant to greet the caller."
        initial_message = {
            ...
            "item": {
                ...
                "role": "user",
                "content": [
                    {"type": "input_text", "text": "A caller has just connected. Greet them with your opening line only."}
                ],
            },
        }
        await azure_ws.send(json.dumps(initial_message))
